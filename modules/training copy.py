...
FINAL_CSV = r"E:\eeg-analysis\eeg-analysis\cognitive_state_discrimination_dataset - cognitive_state_discrimination_dataset.csv"
...
valid_mask = (~X.isna().any(axis=1)) & (y.notna()) & (y.isin(TRAIN_LABELS))
...
for label_val in sorted(y.unique()):
    ...
    pct = count / len(y) * 100
    label_name = LABEL_MAP.get(label_val, str(label_val))
    print(f"  {label_name} ({label_val}): {count} samples ({pct:.1f}%)")


# ============================================================
# DATA SCALING (ALL DATA FOR TRAINING)
# ============================================================
X_train = X.copy()
y_train = y.copy()
...
print("\nData usage:")
print(f"  Train (all valid data): {len(X_train)} samples")
...
print(f"Data Accuracy:     {test_acc:.4f} ({test_acc * 100:.2f}%)")
...
cm = confusion_matrix(y_train, y_train_pred, labels=TRAIN_LABELS)
